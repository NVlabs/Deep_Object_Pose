...
def loadjson(path, objectsofinterest, img):
    ...
    return {
        ...
        "camera_settings":data["camera_data"],
        "class" : obj_class
        }
...
class MultipleVertexJson(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Camera intrinsics should get from image json?
        # Assumes one camera
        cam = data['camera_settings']['intrinsics']

        # Initialize parameters
        matrix_camera = np.zeros((3,3))
        matrix_camera[0,0] = cam["camera_settings"]['fx']
        matrix_camera[1,1] = cam["camera_settings"]['fy']
        matrix_camera[0,2] = cam["camera_settings"]['cx']
        matrix_camera[1,2] = cam["camera_settings"]['cy']
        matrix_camera[2,2] = 1
        ...
        if self.objectsofinterest is None:
            cuboid = np.array(params['dimensions'][0])
        else:
            if self.objectsofinterest in data['class']:
                cuboid = np.array(params['dimensions'][self.objectsofinterest])
